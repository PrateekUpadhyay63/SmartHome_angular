...
export class EventService {
  ...
  constructor(public db: AngularFirestore, private authService: AuthService) {
    let currentUser = this.authService.getUserDetails(); //get current Firebase user object
    this.eventCollection = db.collection<Event>('users/' + currentUser.uid + '/events');
  ...
  getNextAllDayEvents(numberOfEvents) {
    ...
    let currentUser = this.authService.getUserDetails(); //get current Firebase user object
    return this.db.collection<Event>('users/' + currentUser.uid + '/events', ref => ref.where('startTime','>=', currentDate).limit(numberOfEvents)).snapshotChanges().pipe(
      map(actions => {
        return actions.map(a => {
          const data = a.payload.doc.data();
          const id = a.payload.doc.id;
